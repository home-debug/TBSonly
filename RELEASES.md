...
- **modpost "undefined symbol" for cross-module frontend exports on clean builds** - e.g. `saa716x_tbs-dvb.ko` needs `cx24117_get_i2c_adapter`/`tas2101_get_i2c_adapter` from the frontend modules. Hardened the Module.symvers chain: combined file seeded with the kernel's own `$KBUILD/Module.symvers`, deduplicated, and each module dir gets the accumulated symvers copied in before its make run
