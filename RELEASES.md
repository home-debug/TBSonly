...
### Fixes
- **Step order restructured per user request**: fetching TBS sources now happens BEFORE the build-environment check, and `detect_tbs_cards` runs inside the system-check phase (after `detect_distro`/`detect_kernel_sources`), so card info always appears together with the environment summary. Explicit `git` guard added at the fetch step
- **Hardware detection now runs after the sources are fetched** (previously before, so a first-time run silently skipped it); a clear warning is printed when `lspci` (pciutils) is missing instead of a bare "no cards detected"
- **- **Reverted** the previous Module.symvers seeding change (it caused `exported twice` errors for core kernel symbols under modpost) - the build is back to the exact v19 symvers semantics, which built the user's modules successfully
- **Removed `dvb-core` from build targets**: it is a core kernel subsystem already provided by the distro kernel (`dvb-core.ko`); rebuilding it requires full kernel sources and caused `string.h: No such file` and arch header errors on clean builds. TBS frontends link against the existing module
- **Header-patch step hardened**: no longer runs `cp` when the TBS tree is already the source, skips cleanly when the target header is missing, and only backs up/patches a real distro header that exists and differs from TBS
- **REVERTED to main build semantics**: removed all `-I${SRC}/include*` additions and `KBUILD_CPPFLAGS` - restored the exact main/v18 include path and make invocation that worked. All extra features (USB, branch detection, update check, performance patches) are kept but the build core is now main-equivalent
- **Removed `-I${SRC}/include` from include paths**: the TBS tree ships `include/linux/find.h` and other headers that shadow distro `-common` headers and cause `string.h: No such file` errors. Only `-I${SRC}/include/uapi` is kept (needed for TBS-extended `frontend.h`)
- **Include path fix for clean builds**: added `-I${SRC}/include` to `EXTRA_CFLAGS` and `KBUILD_CPPFLAGS` to the module make invocation, so TBS-extended headers (with `modcode`, `set_property`, `read_temp`, `FE_ECP3FW_*`, `FE_24CXX_*`) are found. Previously only `BUILD_DIR/include` was in the search path, but `KCFLAGS` does not reliably inject `-I` for kbuild external modules - the stale-object mask from dirty trees hid this until a from-scratch build
- **Critical header source fix**: `H1`/`H2` (the temporarily-patched frontend headers) now point to the **TBS tree** (`$SRC/include/...`), which carries the required extensions (`modcode`, `set_property`, `read_temp`, `FE_ECP3FW_*`, `FE_24CXX_*`). Previously they pointed at distro headers that either lack the file (Debian arch-specific) or lack the TBS extensions (common) - causing `dvb_frontend.c` to fail with `'struct dtv_frontend_properties' has no member named 'modcode'` on clean builds
- **Detection crash under non-UTF8 locales fixed**: the detector contained an em-dash (`U+2014`) that Python rejected with `SyntaxError: Non-UTF-8 code starting with '0x97'` under a C/POSIX locale - the detector died before printing anything (stderr now goes to the log). Em-dash replaced with ASCII and `# -*- coding: utf-8 -*-` header added; verified by running under `LC_ALL=C`
- **Card detection was silently failing on the real TBS tree** (crash hidden by `2>/dev/null`) - most likely a non-UTF8 byte sequence in one of the parsed TBS sources. File reads are now encoding-safe (`errors="replace"`), any detection error is printed as a visible `WARN|Detection error: ...` line, and stderr goes to the log instead of `/dev/null`. Verified with a regression test including a poisoned non-UTF8 header
- **USB QBox build fixed**: object names corrected to the real source names (`tbs-qbox.o`, `tbs-qbox2.o`, `tbs-qbox22.o`, `tbs-qbox2ci.o`, `tbs-qboxs2.o` - inherited from v19-dev as `tbsqbox*.o`, which broke the whole `usb/dvb-usb` build with "No rule to make target"). All 20 USB module objects now verified against the TBS tree Makefile
